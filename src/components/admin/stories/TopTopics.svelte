<script lang="ts">
  ...
  let topTopics = $state<TopNMetricResult>([]);
  let fetching = $state<boolean>(false);

  const topTopicsMetrics = $state<TopNMetricFilter>({
  ...
  $effect(() => {
    topTopicsMetrics.start_date = dateRange.start_date;
    topTopicsMetrics.end_date = dateRange.end_date;
  });

  const getTopTopics = async () => {
    fetching = true;
    topTopics = await admin.fetchTopTopics(topTopicsMetrics);
    fetching = false;
  };
</script>
...
<div class="flex-row">
  ...
  <div class="container">
    <div class="input-container">
      <label for="topics-top-count">Count</label>
      <input
        id="topics-top-count"
        type="number"
        bind:value={topTopicsMetrics.n}
        min="1"
      />
    </div>
    <button onclick={getTopTopics}>
      {#if fetching}
        <LoadingSVG />
      {:else}
        Fetch Data
...
{#if topTopics.length}
  <ul class="container round-8 fade-in">
    {#each topTopics as { id, name, activity_count }, index}
      <li
        class="flex-row flex-wrap gap-8"
        style:--rank-progress={topTopics.length > 1
