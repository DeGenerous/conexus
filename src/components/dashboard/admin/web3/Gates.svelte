  ...
  <ul class="flex-row flex-wrap">
    {#each gates as gate}
      <li class="orange-tile transition">
        ...
        {#if gate.erc721?.specific_token_ids || gate.erc721?.token_id_min || gate.erc721?.token_id_max || gate.erc20?.min_amount}
          <span class="tile-data">
            {#if gate.gate_kind === 'erc20_token'}
              <p>Minimum Amount:</p>
              <p>{gate.erc20?.min_amount}</p>
            {:else if gate.gate_kind === 'erc721_token'}
              <p>Allowed IDs:</p>
              <p>{gate.erc721?.specific_token_ids?.join(', ')}</p>
            {:else if gate.gate_kind === 'erc721_class'}
              <p>IDs Range:</p>
              <p>{gate.erc721?.token_id_min} - {gate.erc721?.token_id_max}</p>
            {/if}
          </span>
        {/if}
